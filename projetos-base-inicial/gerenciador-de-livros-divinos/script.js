...
function getFilteredBooks() {
  ...
  return tab.items.filter((item) => {
    const matchSearch =
      !searchQ || item.nome.toLowerCase().includes(searchQ.toLowerCase());
    const matchStat =
      !statFilter || item.efeitos.some((e) => e.includes(statFilter));
    const matchOwned =
      ownedFilterMode === "all" ||
...
function renderGrid() {
  ...
  if (!books.length) {
    grid.innerHTML =
      '<div class="empty-state"><div class="empty-icon">📖</div><p>Nenhum livro encontrado.</p></div>';
    return;
  }
  ...
  books.forEach((item) => {
    ...
    const matImgs = item.materiais
      .slice(0, 4)
      .map((m) => {
        if (m.imagem)
          return `<img class="mat-thumb" src="${m.imagem}" alt="${m.nome}" title="${m.nome}">`;
...
function renderOwnedSidebar() {
  ...
  if (!ownedBooks.length) {
    list.innerHTML =
      '<div class="empty-owned">Nenhum livro obtido ainda.</div>';
    return;
  }

  list.innerHTML = ownedBooks
    .map(
      (b) => `
...
function openModal(item) {
  ...
  document.getElementById("modalEffects").innerHTML = item.efeitos
    .map((e) => `<span class="modal-effect-tag">${e}</span>`)
    .join("");

  const mats = document.getElementById("modalMats");
  mats.innerHTML = item.materiais
    .map(
      (m) => `
  ...
  btn.textContent =
    count > 0 ? "✓ Obtido (" + count + ") - Remover 1" : "+ Marcar como Obtido";
  ...
  let totalItems = 0;
  Object.values(total).forEach((v) => (totalItems += v));
  let remItems = 0;
  Object.values(rem).forEach((v) => (remItems += v));
  let pct =
    totalItems > 0
      ? Math.round(((totalItems - remItems) / totalItems) * 100)
      : 100;
  ...
  if (totalItems > 0 && item.nivel > 1) {
    ...
    if (pct === 100)
      document.getElementById("modalProgressBar").style.background = "#88c5ff";
...
document.querySelectorAll("#ownedFilter .pill").forEach((p) => {
  p.onclick = () => {
    document
      .querySelectorAll("#ownedFilter .pill")
...
document.getElementById("btnResetTree").onclick = () => {
  if (
    currentTreeRoot &&
    confirm(
      `Deseja resetar todo o progresso marcado para a árvore de "${currentTreeRoot}"?`,
    )
  ) {
...
function openTree(book) {
  ...
  owned.forEach((x) => {
    if (!x.startsWith("@"))
      currentGlobalPool[x] = (currentGlobalPool[x] || 0) + 1;
  });

  document.getElementById("craftTreeContainer").innerHTML = buildTreeNodeHtml(
    book.nome,
    1,
    book.nome,
  );
...
function buildTreeNodeHtml(bookName, qty, path) {
  ...
  const imgUrl = book
    ? book.imagem
    : materialImageMap[bookName] || "images/fragmentodeescritadivina.jpg";
  ...
  if (book && book.materiais && book.materiais.length) {
    ...
    book.materiais.forEach((m, idx) => {
      ...
      if (
        m.nome !== "Fragmentos de Escrita Divina" &&
        m.nome !== "Livros em Branco Divino"
      ) {
        childHtmlNodes.push(
          `<div class="tree-child-wrapper">${buildTreeNodeHtml(m.nome, m.quantidade || 1, path + "-" + idx)}</div>`,
        );
      }
    });
...
function bindTreeEvents() {
  document.querySelectorAll(".tree-node").forEach((node) => {
    ...
    node.onclick = (e) => {
      ...
      if (isSpec) {
        owned = owned.filter(
          (x) => x !== specificKey && !x.startsWith("@" + path + "-"),
        );
      } else if (isGlob) {
      ...
      const rootName = document
        .getElementById("treeTitleName")
        .textContent.replace("Árvore: ", "");
      currentGlobalPool = {};
      owned.forEach((x) => {
        if (!x.startsWith("@"))
          currentGlobalPool[x] = (currentGlobalPool[x] || 0) + 1;
      });
      document.getElementById("craftTreeContainer").innerHTML =
        buildTreeNodeHtml(rootName, 1, rootName);
      bindTreeEvents();
...
function updateTreeCosts(bookName) {
  ...
  if (Object.keys(rem).length === 0) {
    ...
    Object.keys(rem).forEach((matName) => {
      const bookRef = allBooks.find(
        (b) => b.materiais && b.materiais.some((m) => m.nome === matName),
      );
      const matInfo = bookRef
        ? bookRef.materiais.find((m) => m.nome === matName)
        : null;
  ...
  if (pct === 100) bar.style.background = "#88c5ff";
    ...
    bar.style.background = "linear-gradient(90deg, var(--gold-dark), #88c5ff)";
  ...
  function traverse(bName, path, qtyInNode) {
    if (!listItems[bName]) {
      ...
      listItems[bName] = {
        ...
        image: bObj
          ? bObj.imagem
          : materialImageMap[bName] || "images/fragmentodeescritadivina.jpg",
